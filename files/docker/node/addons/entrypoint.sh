...
if [[ "$NETWORK" == "mainnet" ]]; then
  $CNODE_HOME/scripts/prereqs.sh -n mainnet -t cnode -s > /dev/null 2>&1 \
  && customise \
  && exec $CNODE_HOME/scripts/cnode.sh
elif [[ "$NETWORK" == "testnet" ]]; then
  $CNODE_HOME/scripts/prereqs.sh -n testnet -t cnode -s > /dev/null 2>&1 \
  && customise \
  && exec $CNODE_HOME/scripts/cnode.sh
elif [[ "$NETWORK" == "preprod" ]]; then
  $CNODE_HOME/scripts/prereqs.sh -n preprod -t cnode -s > /dev/null 2>&1 \
  && customise \
  && exec $CNODE_HOME/scripts/cnode.sh
elif [[ "$NETWORK" == "preview" ]]; then
  $CNODE_HOME/scripts/prereqs.sh -n preview -t cnode -s > /dev/null 2>&1 \
  && customise \
  && exec $CNODE_HOME/scripts/cnode.sh
  ...
  $CNODE_HOME/scripts/prereqs.sh -n guild -t cnode -s > /dev/null 2>&1 \
  && customise \
  && exec $CNODE_HOME/scripts/cnode.sh
else
